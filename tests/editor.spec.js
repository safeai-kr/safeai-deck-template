...
test('리사이즈 — inline max-width 텍스트도 가로로 확대됨(bake 시 max-width 해제)', async ({ page }) => {
  // 재현: 제목/문단에 가독성용 max-width 를 준 텍스트는 bake 후에도 max-width 가
  // 남아, style.width 를 키워도 렌더 폭이 캡에 걸려 안 늘어났다.
  const res = await page.evaluate(() => {
    const E = window.__deckEditor;
    E.setMode(true);
    const slide = E.activeSlide();
    const unit = E.collectUnits(slide)
      .find((u) => u.textContent.trim() && u.tagName !== 'IMG' && u.tagName !== 'SVG');
    unit.style.maxWidth = '200px';        // 가독성용 max-width 시뮬레이션
    E.bakeSlide(slide);                   // 리사이즈 핸들이 트리거하는 것과 동일
    const before = unit.getBoundingClientRect().width;
    // 리사이즈가 하는 것과 동일하게 style.width 를 캡 너머로 키운다
    unit.style.width = ((parseFloat(unit.style.width) || 0) + 400) + 'px';
    const after = unit.getBoundingClientRect().width;
    return { maxW: getComputedStyle(unit).maxWidth, before, after };
  });
  expect(res.maxW).toBe('none');                       // bake 가 max-width 를 해제
  expect(res.after).toBeGreaterThan(res.before + 100); // 렌더 폭이 실제로 커짐
});
